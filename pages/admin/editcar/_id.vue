...
export default {
  ...
  mounted() {
    ...
    this.value = re.type;
    this.model = re.model;
    this.imageUrl = "http://api.albasheerrentcar.com/demo/ecom/storage/" + re.image;
    this.size = re.size;
    ...
    if (re.photos) {
      re.photos.forEach((element) => {
        this.imageUrls.push('http://api.albasheerrentcar.com/demo/ecom/storage/'+element.path);
      });
    }
    this.id = re.slug;
    this.idd = re.id
